<?php
  ...
  if(isset($_GET['student']) && isset($_GET['hostel'])){
    $student_id = $_GET['student'];
    $hostel_name = $_GET['hostel'];

    // Get hostel_id
    $query_h = "SELECT Hostel_id FROM Hostel WHERE Hostel_name = '$hostel_name'";
    $result_h = mysqli_query($conn, $query_h);
    if(mysqli_num_rows($result_h) == 0){
      echo "<script type='text/javascript'>alert('Invalid hostel')</script>";
      exit();
    }
    $row_h = mysqli_fetch_assoc($result_h);
    $hostel_id = $row_h['Hostel_id'];

    // Check if application exists
    $query_app = "SELECT * FROM Application WHERE Student_id = '$student_id' AND Hostel_id = '$hostel_id' AND Application_status = 1";
    $result_app = mysqli_query($conn, $query_app);
    if(mysqli_num_rows($result_app) == 0){
      echo "<script type='text/javascript'>alert('No pending application found')</script>";
      exit();
    }

    // Find empty room
    $query_room = "SELECT * FROM Room WHERE Allocated = '0' AND Hostel_id = '$hostel_id' ORDER BY Room_No LIMIT 1";
    $result_room = mysqli_query($conn, $query_room);
    if(mysqli_num_rows($result_room) == 0){
      echo "<script type='text/javascript'>alert('No rooms available')</script>";
      exit();
    }
    ...
    // Update Application
    $query_up_app = "UPDATE Application SET Application_status = '0', Room_No = '$room_no' WHERE Student_id = '$student_id' AND Hostel_id = '$hostel_id'";
    $result_up_app = mysqli_query($conn, $query_up_app);

    if($result_up_app){
      // Update Student
      $query_up_stu = "UPDATE Student SET Hostel_id = '$hostel_id', Room_id = '$room_id' WHERE Student_id = '$student_id'";
      $result_up_stu = mysqli_query($conn, $query_up_stu);

      if($result_up_stu){
        // Update Room
        $query_up_room = "UPDATE Room SET Allocated = '1' WHERE Room_id = '$room_id'";
        $result_up_room = mysqli_query($conn, $query_up_room);

        if($result_up_room){
          echo "<script type='text/javascript'>alert('Room allocated successfully')</script>";
        } else {
          echo "<script type='text/javascript'>alert('Failed to update room')</script>";
        }
      } else {
    ...
    echo "<script>window.location.href='../admin/applications.php';</script>";
    exit();
  }
	...
	<header>
		<div class="container agile-banner_nav">
			<nav class="navbar navbar-expand-lg navbar-light bg-light">
				...
				<h1><a class="navbar-brand" href="home_manager.php">VGI<span class="display"></span></a></h1>
...
<footer class="py-5">
	<div class="container py-md-5">
		<div class="footer-logo mb-5 text-center">
			<a class="navbar-brand"  href="#" target="_blank" >VGI<span class="display"></span></a>
		</div>
